...
class ThreadTestCase(TestCase):
    ...
    def test_add_messages_to_thread(self):
        self.thread.users.add(self.user1, self.user2)
        messages1 = Message.objects.create(user=self.user1, content="Muy buenas")
        messages2 = Message.objects.create(user=self.user2, content="Hola")
        self.thread.messages.add(messages1, messages2)
    ...
    def test_add_message_from_user_not_in_thread(self):
        ...
        messages1 = Message.objects.create(user=self.user1, content="Muy buenas")
        messages2 = Message.objects.create(user=self.user2, content="Hola")
        messages3 = Message.objects.create(user=self.user3, content="Soy un espia")

        self.thread.messages.add(messages1, messages2, messages3)
        self.assertEqual(len(self.thread.messages.all()), 2)
